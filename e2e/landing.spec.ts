...
test.describe('Landing Page', () => {
  test('guest should see heading, description, and Login/Register CTAs', async ({ page }) => {
    ...
    await expect(page.getByText(/personal task management board/i)).toBeVisible();
  ...
  test('authenticated user should see "Go to Board" link instead of Login/Register', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: /go to board/i })).toBeVisible();
  });
});
