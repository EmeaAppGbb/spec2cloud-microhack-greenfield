# Chat Interface — Split Panel with Streaming
...
- [Split-panel layout renders on wide viewport](#split-panel-layout-renders-on-wide-viewport)
- [Panels stack vertically on narrow viewport](#panels-stack-vertically-on-narrow-viewport)
- [User submits a non-empty message with Enter key](#user-submits-a-non-empty-message-with-enter-key)
- [Submit is disabled when input is empty](#submit-is-disabled-when-input-is-empty)
- [Shift+Enter inserts a newline instead of submitting](#shift-enter-inserts-a-newline-instead-of-submitting)
- [Input is disabled while the pipeline is active](#input-is-disabled-while-the-pipeline-is-active)
- [Text tokens appear progressively in chat](#text-tokens-appear-progressively-in-chat)
- [Chat auto-scrolls to latest content when user has not scrolled up](#chat-auto-scrolls-to-latest-content-when-user-has-not-scrolled-up)
- [Auto-scroll pauses when user scrolls up](#auto-scroll-pauses-when-user-scrolls-up)
- [Explanatory text streams then plan block appears atomically](#explanatory-text-streams-then-plan-block-appears-atomically)
- [Plan block displays all seven fields](#plan-block-displays-all-seven-fields)
