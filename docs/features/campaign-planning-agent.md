...
- [Planner receives the full brief text](#planner-receives-the-full-brief-text)
- [Leading and trailing whitespace is trimmed from brief](#leading-and-trailing-whitespace-is-trimmed-from-brief)
- [Empty brief is rejected with a prompt message](#empty-brief-is-rejected-with-a-prompt-message)
- [Brief shorter than 10 characters is rejected](#brief-shorter-than-10-characters-is-rejected)
- [Brief with exactly 9 characters is rejected](#brief-with-exactly-9-characters-is-rejected)
- [Brief with exactly 10 characters is accepted](#brief-with-exactly-10-characters-is-accepted)
- [Brief with 13 characters is accepted](#brief-with-13-characters-is-accepted)
- [Brief exceeding 2000 characters is truncated with notification](#brief-exceeding-2000-characters-is-truncated-with-notification)
- [Brief within limit is used in full without truncation](#brief-within-limit-is-used-in-full-without-truncation)
- [Brief at exactly 2000 characters is used in full](#brief-at-exactly-2000-characters-is-used-in-full)
- [Plan contains all seven required fields](#plan-contains-all-seven-required-fields)
- [Key messages list is padded to minimum of 2 items](#key-messages-list-is-padded-to-minimum-of-2-items)
- [Key messages list is capped at maximum of 5 items](#key-messages-list-is-capped-at-maximum-of-5-items)
- [Key messages within valid range are unchanged](#key-messages-within-valid-range-are-unchanged)
- [Defaults applied when brief does not specify platform, audience, or tone](#defaults-applied-when-brief-does-not-specify-platform-audience-or-tone)
- [Explicit values in brief override smart defaults](#explicit-values-in-brief-override-smart-defaults)
- [Partial defaults applied when only some values are specified](#partial-defaults-applied-when-only-some-values-are-specified)
- [Explanatory text streams followed by atomic plan block](#explanatory-text-streams-followed-by-atomic-plan-block)
- [SSE stream delivers tokens then structured plan event](#sse-stream-delivers-tokens-then-structured-plan-event)
- [Planner signals handoff to Creative Generator after plan completion](#planner-signals-handoff-to-creative-generator-after-plan-completion)
- [Timeline updates from Planning to Generating on handoff](#timeline-updates-from-planning-to-generating-on-handoff)
- [Transparent retry on first LLM failure](#transparent-retry-on-first-llm-failure)
- [Transparent retry on second LLM failure](#transparent-retry-on-second-llm-failure)
- [Error and retry button after all 3 attempts fail](#error-and-retry-button-after-all-3-attempts-fail)
- [Retry button reuses the original brief](#retry-button-reuses-the-original-brief)
